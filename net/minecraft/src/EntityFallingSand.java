        ...
        if (!worldObj.isRemote)
        {
            ...
            if (onGround)
            {
                ...
                if (worldObj.getBlockId(i, j, k) != Block.pistonMoving.blockID)
                {
                    ...
                    if ((!worldObj.canBlockBePlacedAt(blockID, i, j, k, true, 1) || BlockSand.canFallBelow(worldObj, i, j - 1, k) || !worldObj.setBlockAndMetadataWithNotify(i, j, k, blockID, field_56185_b)) && !worldObj.isRemote)
                    {
                        entityDropItem(new ItemStack(blockID, 1, field_56185_b), 0.0F);
                    }
                }
            }
