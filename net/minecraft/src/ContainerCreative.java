...
class ContainerCreative extends Container
{
    ...
    public ContainerCreative(EntityPlayer par1EntityPlayer)
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            for (int k = 0; k < 9; k++)
            {
                func_55197_a(new Slot(GuiContainerCreative.getInventory(), i * 9 + k, 9 + k * 18, 18 + i * 18));
            }
        }

        for (int j = 0; j < 9; j++)
        {
            func_55197_a(new Slot(inventoryplayer, j, 9 + j * 18, 112));
        }
