...
public class PanelCrashReport extends Panel
{
    public PanelCrashReport(CrashReport par1CrashReport)
    {
        ...
        par1CrashReport.func_55363_b().printStackTrace(new PrintWriter(stringwriter));
        ...
        try
        {
            ...
            s2 = (new StringBuilder()).append(s2).append(par1CrashReport.func_55372_c()).toString();
        ...
        else
        {
            ...
            s3 = (new StringBuilder()).append(s3).append("      ----------------------      \n").toString();
            s3 = (new StringBuilder()).append(s3).append("\n").toString();
            s3 = (new StringBuilder()).append(s3).append("Minecraft has stopped running because it encountered a problem; ").append(par1CrashReport.func_55365_a()).append("\n").toString();
            File file = par1CrashReport.func_57261_f();

            if (file == null)
            {
                par1CrashReport.func_55370_a(new File(new File(Minecraft.getMinecraftDir(), "crash-reports"), (new StringBuilder()).append("crash-").append((new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss")).format(new Date())).append("-client.txt").toString()));
                file = par1CrashReport.func_57261_f();
            }
